        ...
        for gammaStaticIndex = 1 : length(gammaStatic)                           % Gamma range for static - 2nd dim
            for positionIndex = 1 : length(position)                       % position range - 3nd dim
                for velocityIndex = 1 : length(velocity)                   % velocity range - 4th dim
                    ...
                    fileName = [saveDirectory,'/dataGammaD',zeroPad2String(gammaDynamic(gammaDynamicIndex),3),...
                        'GammaS',zeroPad2String(gammaStatic(gammaStaticIndex),3),...
                        'Position',zeroPad2String(position(positionIndex),3),...
                        'Velocity',zeroPad2String(velocity(velocityIndex),3),'.mat'];
